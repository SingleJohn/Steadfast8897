...
- T21/T22 CSP stdio 编码修复：JVM sidecar JSON-lines stdin/stdout 统一强制 UTF-8，Go 启动 JVM 补 UTF-8 参数，修正中文 Windows 默认 cp936 导致的 RPC 边界乱码。
...
- CSP sidecar 新增 UTF-8 `RpcIO`，`CSPProbe`、`okhttp3` bridge、CatVod `OkHttp` 桩所有 JSON-lines 读写都走同一套 UTF-8 stdin/stdout，不再使用裸 `System.out.println` 或重复包裹 `System.in`。
- Go 启动 JVM sidecar 时增加 `-Dfile.encoding=UTF-8`、`-Dsun.stdout.encoding=UTF-8`、`-Dsun.stderr.encoding=UTF-8`，双侧固定 RPC 字符集，避免中文 Windows 默认 cp936 破坏 UTF-8 JSON。
- CSP HTTP bridge 保留文本响应判定与多级编码探测：优先原样透传合法 UTF-8；仅在响应明确声明或检测到非 UTF-8 时转码；二进制响应只保留 `bodyBase64`，避免把非 UTF-8 字节直接交给 Go `json.Marshal`。
