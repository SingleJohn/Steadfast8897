...
实际落点：新增 internal/source/ids.go、internal/source/entity.go、internal/repository/source_repository.go、internal/models/source.go；扩展 internal/repository/item_helper_repository.go 的 ItemExists 与 internal/repository/repository.go 的 Source 聚合入口；实现 source_item/play_source/source_view/episode 确定性 UUID、source_* CRUD/upsert、ResolveEntity 分派；T2 commit 范围：5a79672..bd7dee6。
...
实际落点：新增 web/src/api/source.ts、web/src/composables/useSourceCenter.ts、web/src/pages/SourceCenterPage.vue、web/src/components/source-center/SourceImportPanel.vue、SourceProviderPanel.vue、SourceViewsPanel.vue；扩展 web/src/router.ts 新增后台网关菜单 /admin/gateway/source-center；实现 TVBox URL/粘贴 JSON 导入、配置/Provider 列表与启停、Provider 探活/分类/搜索测试、在线虚拟库创建/编辑/删除/维度值发现/排序/封面生成与清除/expose_to_emby 开关；前端按 Composition API 与领域组件拆分；T8 commit：c7c4a38。
...
实际落点：新增 internal/repository/source_gc_repository.go、internal/services/source_gc.go，并在 main.go 启动 source_items 后台 GC；GC 读取 source_items_gc_retention_days/source_items_gc_interval_hours/source_items_gc_batch_size，默认 90 天/24 小时/500 条，删除 last_seen_at 过期且未命中 expose 在线虚拟库的 source_items，source_play_sources/source_user_item_data 依赖外键级联；新增 internal/source/observability.go，扩展 internal/services/log_router.go、log_target.go 的 source/provider/resolver 日志分类，补充 Provider 分类/搜索/详情/健康、ResolveEntity、ResolvePlay、/SourcePlay 代理流的 provider_id/action/latency/status/error_type/cache_hit/upstream_status/url_hash 结构化日志，播放 URL 仅记 hash；T9 commit 范围：d99c7ec..76b15e9。
