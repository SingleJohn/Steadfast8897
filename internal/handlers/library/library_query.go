...
func getUserViews(c *gin.Context) {
	...
	if !shared.MatchUserOrAdmin(c, userID) {
		c.JSON(http.StatusForbidden, gin.H{"message": "Forbidden"})
		return
	}

	ctx := c.Request.Context()
	scope, err := shared.LoadUserLibraryScope(ctx, state, userID)
	if err != nil {
	...
	if scope.AllowAll && models.IsPlatformLibrariesEnabled(ctx, state.DB) {
		...
		for _, p := range platforms {
			...
			entry = embysupport.ApplyCollectionFolderDefaults(entry, colType, colType != "")
	...
	for _, lib := range libs {
		...
		entry = embysupport.ApplyCollectionFolderDefaults(entry, lib.CollectionType, lib.CollectionType != "mixed")
		if lib.CollectionType == "mixed" {
			if ud, ok := entry["UserData"].(gin.H); ok {
				delete(ud, "PlayCount")
				delete(ud, "UnplayedItemCount")
...
func queryAny(c *gin.Context, keys ...string) string {
	return shared.QueryAny(c, keys...)
}

func parseCompatBool(value string) (bool, error) {
	return shared.ParseCompatBool(value)
}
...
func normalizeItemType(s string) string {
	return shared.NormalizeItemType(s)
}
...
func resolvePhysicalParentForItems(ctx context.Context, state *AppState, scope *shared.UserLibraryScope, opts *models.ItemQueryOptions) (bool, error) {
...
func getLatestItems(c *gin.Context) {
	...
	if needMediaSources {
		if _, ok := models.ResolvePlatformVirtualID(ctx, state.DB, parentID); !ok {
			...
			compathandlers.ApplyUnplayedItemCounts(ctx, state.DB, pathUser, items)
			c.JSON(http.StatusOK, embysupport.BaseItemsToEmbyMaps(items))
			return
		}
	}
...
func queryLatestItemsForParent(ctx context.Context, state *AppState, parentID string, limit int64, scope *shared.UserLibraryScope, userID string, platformByID map[string]models.PlatformLibrary) ([]dto.BaseItemDto, error) {
	...
	if platform != nil {
		...
		compathandlers.ApplyUnplayedItemCounts(ctx, state.DB, userID, items)
		return items, nil
	}
...
func getLatestBatch(c *gin.Context) {
	...
	for id, items := range result {
		out[id] = embysupport.BaseItemsToEmbyMaps(items)
	}
