...
func deviceIDFromRequest(c *gin.Context) string {
	return shared.StrOrPtr(middleware.GetAuthInfo(c).DeviceID, "unknown")
}
...
func OnPlaybackStart(c *gin.Context) {
	...
	activePlaybacks[playbackKey(auth.ID, deviceID)] = &activePlayback{
		...
		deviceName:        deviceName,
		deviceID:          deviceID,
		appVersion:        shared.StrOrPtr(info.Version, ""),
		clientIP:          clientIP,
		userAgent:         userAgent,
		playMethod:        playMethod,
	...
	services.EmitPlaybackNotify(
		...
		auth.ID,
		auth.Name,
		buildNotifySession(clientIP, clientName, deviceName, deviceID, shared.StrOrPtr(info.Version, ""), body.PlaySessionId),
