...
- **封面生成**：复用 `internal/services/coverart` 的多风格 registry（`/Library/CoverArt/Styles` 列出 ninegrid/showcase…），`POST /Library/Platforms/:id/Image/Generate` 与 `.../CoverArt/GenerateAll` 接收 `{Style, Options}`。新增风格只需 `coverart.Register`，前后端下拉自动出现。`DELETE /Library/Platforms/:id/Image` 清除生成封面，内置平台(Netflix 等)回退到 `images.go` 的 `servePlatformLogo`(内置 logo)。
